...
test.describe('Accessibility Tree Validation', () => {
  ...
  test('theme toggle button should have descriptive accessible name', async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
    ...
    const themeToggle = buttons.find(b => b.name && b.name.toLowerCase().includes('mode'));
  ...
  test('nav toggle should have accessible name and state', async ({ page }) => {
    ...
    const navToggle = buttons.find(b =>
      b.name && (b.name.toLowerCase().includes('navigation') || b.name.toLowerCase().includes('menu'))
    );
  ...
  test('logo link should have accessible name', async ({ page }) => {
    ...
    const logoLink = links.find(l =>
      l.name && (l.name.toLowerCase().includes('1g of code') || l.name.toLowerCase().includes('home'))
    );
  ...
  test('episodes page should have valid heading hierarchy', async ({ page }) => {
  ...
  test('individual episode page should have valid heading hierarchy', async ({ page }) => {
  ...
  test('lists should be properly structured', async ({ page }) => {
    ...
    for (const list of lists) {
      if (list.children && list.children.length > 0) {
        const hasListItems = list.children.some((child: any) => child.role === 'listitem');
  ...
  test('nav toggle should communicate expanded state', async ({ page }) => {
    ...
    const initialSnapshot = await (page as PageWithAccessibility).accessibility.snapshot();
    const initialButtons = findByRole(initialSnapshot, 'button');
    const initialNavToggle = initialButtons.find(b =>
      b.name && (b.name.toLowerCase().includes('navigation') || b.name.toLowerCase().includes('menu'))
    );
    ...
    const openSnapshot = await (page as PageWithAccessibility).accessibility.snapshot();
    const openButtons = findByRole(openSnapshot, 'button');
    const openNavToggle = openButtons.find(b =>
      b.name && (b.name.toLowerCase().includes('navigation') || b.name.toLowerCase().includes('menu'))
    );
  ...
  test('mobile menu content should be accessible when open', async ({ page }) => {
    await page.setViewportSize({ width: 375, height: 667 });
    await page.goto('/');
    await page.waitForLoadState('networkidle');
    ...
    const navLinks = links.filter(l =>
      l.name === 'Home' || l.name === 'Episodes' || l.name === 'About'
    );

    expect(navLinks.length).toBeGreaterThanOrEqual(2);
  });

  test('mobile menu content should not be in tree when closed', async ({ page }) => {
    await page.setViewportSize({ width: 375, height: 667 });
    await page.goto('/');
    await page.waitForLoadState('networkidle');
