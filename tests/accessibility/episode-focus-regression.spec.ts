...
test.describe('Episode Focus Indicator Regression Tests', () => {
  test('episode card should not have overflow hidden that clips the outline', async ({ page }) => {
  ...
  test('episode link focus outline should be fully visible (not clipped)', async ({ page }) => {
    await page.goto('/episodes/');
    await page.waitForLoadState('networkidle');
  ...
  test('episode link focus outline should be visible in both light and dark modes', async ({ page }) => {
  ...
  test('thumbnail should still have overflow hidden for image clipping', async ({ page }) => {
    await page.goto('/episodes/');
    await page.waitForLoadState('networkidle');
  ...
  test('focus outline should be visible on episode cards on home page', async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
    ...
    if (episodeLinks.length > 0) {
      ...
      const hasFocusIndicator = await firstLink.evaluate((el) => {
        const computed = window.getComputedStyle(el);
        const hasOutline = computed.outlineWidth !== '0px' &&
  ...
  test('focus outline offset should not be clipped by parent containers', async ({ page }) => {
    await page.goto('/episodes/');
    await page.waitForLoadState('networkidle');
