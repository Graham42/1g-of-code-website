...
test.describe('Keyboard Navigation', () => {
  test('skip link should be visible on focus and work correctly', async ({
    page,
  }) => {
    await page.goto('/')
  ...
  test('tab order on mobile should include nav toggle', async ({ page }) => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (info.className.includes('skip-link')) {
        ...
        info.className.includes('nav-toggle') ||
        info.ariaLabel.includes('navigation')
      ) {
    ...
    let focused = await firstNavLink.evaluate(
  ...
  test('all interactive elements should be focusable', async ({ page }) => {
    ...
    const interactiveElements = await page
      .locator(
        '.site-wrapper a, .site-wrapper button, .site-wrapper input, .site-wrapper select, .site-wrapper textarea'
      )
      .all()
